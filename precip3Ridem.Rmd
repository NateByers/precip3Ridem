...
it is a sequence of integers. And at the end you advance the index variable so
that it takes the next value in the sequence (in most cases, it is just the next
integer, `+ 1`).
...
So we created the index variable `i` with the initial value of `1`, created the
...
And the last line of the loop adds a `1` to the index variable, so `i` takes on 
